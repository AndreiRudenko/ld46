...
class SamplesDebugView extends DebugView {
	...
	public function new(_debug:Debug) {
		...
		Clay.layers.add(itemsList, clay.render.Layers.DEBUG_UI);
	...
	override function onDisabled() {
		...
		Clay.off(MouseEvent.MOUSE_WHEEL, onMouseWheel);
	...
	inline function listChannel(_list:StringBuf, c:AudioChannel, _depth:Int = 1) {
		if(Std.is(c, WavetableOsc)) {
			var s:WavetableOsc = cast c;
			_objIndex++;
			...
			var freqs = [];
			for (o in s.oscs) {
				freqs.push(Math.round(o.frequency));
			}
			bar.text = '[${freqs.join(',')}]';
			bar.max = 1;
			bar.value = s.volume;
